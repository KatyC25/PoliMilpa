...
> = {
	verde: {
		label: "Apto para siembra",
		icon: "fa-circle-check",
		color: "#1a9850",
	},
	amarillo: {
		label: "Precaución — esperar 7 días",
		icon: "fa-circle-exclamation",
		color: "#fdae61",
	},
	rojo: {
		label: "No apto para siembra",
...
function NinoBanner({
	...
	alert: { level: string; label: string; message: string };
}) {
	...
	return (
		<div className="fd3-fos">
			...
			<div className="fd3-fos-grid">
				{all.map((w) => {
					const meta =
						FOS_STATUS_META[w.status] ?? FOS_STATUS_META["no_aplica"];
					const datesCls =
						w.status === "activa"
							? "fd3-fos-dates--activa"
							: w.status === "proxima"
								? "fd3-fos-dates--proxima"
								: "";
					const datesLabel =
						w.status === "activa"
							? `${w.inicio} – ${w.fin}`
			...
			<div className="fd3-fos-legend">
				<span className="fd3-fos-legend-item">
					<span className="fd3-fos-status fd3-fos-status--activa" /> Activa
				</span>
				<span className="fd3-fos-legend-item">
					<span className="fd3-fos-status fd3-fos-status--proxima" /> Próxima
				</span>
				<span className="fd3-fos-legend-item">
					<span className="fd3-fos-status fd3-fos-status--expirada" /> Cerrada
					(ventana paso)
				</span>
				<span className="fd3-fos-legend-item">
					<span className="fd3-fos-status fd3-fos-status--no_aplica" /> No
					aplica en zona
				</span>
			</div>
		</div>
				...
				<div className="dash-nav">
					{user && (
						<span className="dash-user">
							<i className="fa-regular fa-user" /> {user.full_name}
						</span>
					)}
				</div>
			</header>
			...
			<section className="dash-content">
				{error ? (
					<div className="dash-empty">
						...
						<p>{error}</p>
						<button className="dash-retry" onClick={handleBack} type="button">
							Volver
						</button>
					</div>
				) : farmer ? (
						...
						<div className="fd3-head">
							<div>
								...
								<p className="fd3-head-sub">
									{farmer.full_name} &middot; {farmer.municipality},{" "}
									{farmer.department} &middot; {farmer.farmer_code}
								</p>
							</div>
								...
								{rec ? (
									<>
										<div
											...
											style={{
												backgroundColor:
													TRAFFIC_META[rec.traffic_light]?.color ?? "#999",
											}}
										>
											<i
												className={`fa-solid ${TRAFFIC_META[rec.traffic_light]?.icon}`}
											/>
											<div className="fd3-traffic-body">
												<strong>
													{TRAFFIC_META[rec.traffic_light]?.label}
												</strong>
												<span>
													Score global:{" "}
													{(typeof rec.debug_scores?.global === "number"
														? rec.debug_scores.global
														: 0
													).toFixed(3)}
												</span>
											</div>
										...
										<div className="fd3-crops">
											<div className="fd3-crop fd3-crop--renta">
												<div className="fd3-crop-icon">
													<i className="fa-solid fa-seedling" />
												</div>
												<span className="fd3-crop-label">Renta</span>
												<span className="fd3-crop-name">
													{rec.recommendations[0]?.rent_crop ?? "—"}
												</span>
											</div>
											<div className="fd3-crop fd3-crop--food">
												...
												<span className="fd3-crop-name">
													{rec.recommendations[0]?.food_crop ?? "—"}
												</span>
											</div>
											<div className="fd3-crop fd3-crop--window">
												...
												<span className="fd3-crop-name">
													{rec.recommended_window.replace(/_/g, " ")}
												</span>
											</div>
										</div>
									</>
								...
								{rec && (
									<div className="fd3-tech">
										...
										>
											...
											<i
												className={`fa-solid fa-chevron-down fd3-tech-chevron ${showTech ? "open" : ""}`}
											/>
										</button>
											...
											<div className="fd3-tech-body">
												<div className="fd3-tech-grid">
													{Object.entries(rec.debug_scores).map(([k, v]) => (
														<div key={k} className="fd3-tech-item">
															...
															<span
																className={`fd3-tech-val ${typeof v === "string" ? "fd3-tech-str" : ""}`}
															>
																{typeof v === "number"
																	? v.toFixed(3)
																	: String(v)}
												...
												{rec.data_source && (
													<p className="fd3-tech-src">
														<i className="fa-solid fa-satellite" /> Fuente:{" "}
														{rec.data_source}
													</p>
												)}
											</div>
							...
							<div className="fd3-right">
								{rec ? (
									<>
											...
											{aiAdvisory ? (
												<>
													...
													<div className="fd3-ai-actions">
														...
														>
															<i
																className={`fa-solid ${editing ? "fa-check" : "fa-pen"}`}
															/>
													...
													<div>
														...
														<p>
															{aiError
																...
																: "Completa el análisis satelital primero."}
														</p>
													</div>
												</div>
											)}
										...
										{c3sValue &&
										typeof c3sValue === "string" &&
										C3S_META[c3sValue] ? (
											<div className="fd3-c3s">
												<div className="fd3-c3s-head">
													<i
														className={`fa-solid ${C3S_META[c3sValue].icon}`}
														style={{ color: C3S_META[c3sValue].color }}
													/>
													<h3>Pronóstico de precipitación</h3>
												</div>
												<div className="fd3-c3s-body">
													<span
														className="fd3-c3s-badge"
														style={{
															backgroundColor: C3S_META[c3sValue].color,
														}}
													>
														{C3S_META[c3sValue].label}
													</span>
													<p className="fd3-c3s-src">
